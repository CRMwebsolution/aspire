...
export default async function DashboardPage() {
  ...
  if (!canUseAdminDashboard) {
    ...
    const calendarData: DashboardData = {
      ...
      appointments: (appointments.data ?? []).filter((item) => item.kind !== "blocked"),
  ...
  const [customers, catalog, rewards, loyalty, appointments, assignments, services, employees, points, inquiries] = await Promise.all([
    supabase.from("aspire_customers").select("id,business_key,full_name,phone,email,phone_normalized,vehicle_details,vehicles,notes,starting_points,created_at,updated_at").eq("business_key", ASPIRE_BUSINESS_KEY).order("full_name"),
    supabase.from("aspire_catalog_items").select("id,business_key,section,slug,level_label,name,summary,primary_price,primary_price_label,secondary_price,secondary_price_label,is_starting_at,is_quote_only,is_featured,is_active,sort_order,features,duration_minutes,draft_content,published_at,created_at,updated_at").eq("business_key", ASPIRE_BUSINESS_KEY).order("section").order("sort_order"),
    ...
    supabase.from("aspire_appointments").select("id,business_key,customer_id,inquiry_id,catalog_item_id,assigned_user_id,kind,status,title,starts_at,ends_at,all_day,location,vehicle_details,final_amount,notes,created_at,updated_at").eq("business_key", ASPIRE_BUSINESS_KEY).order("starts_at"),
    supabase.from("aspire_appointment_assignments").select("business_key,appointment_id,user_id,assigned_at").eq("business_key", ASPIRE_BUSINESS_KEY),
    supabase.from("aspire_appointment_services").select("appointment_id,catalog_item_id,sort_order"),
    supabase.from("aspire_employee_access").select("business_key,user_id,email,display_name,role,is_active,is_hidden").eq("business_key", ASPIRE_BUSINESS_KEY).eq("is_hidden", false).in("role", ["employee", "admin"]).order("display_name"),
  ...
  const firstError = [customers, catalog, rewards, loyalty, appointments, assignments, services, employees, points, inquiries].find((result) => result.error)?.error;
